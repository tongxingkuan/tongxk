<template>
  ...
  <div class="wrapper">
        ...
        <el-scrollbar>
          ...
          <div v-for="item in jsLoadedImageList" :key="item">
            ...
            <i style="margin: 0 5px">{{
              "图片顶部 = " + item.getBoundingClientRect().top
            }}</i>
            <i style="margin: 0 5px">{{
              "视口底部 = " + jsRef.getBoundingClientRect().bottom
            }}</i>
          </div>
        </el-scrollbar>
      </div>
    ...
    <div class="lazyload-intersectionObserve">
      ...
      <div class="explain">
        <el-scrollbar>
          上面我们利用BoundingClientRect获取元素的top属性和滚动元素的bottom属性比较，有一个缺点是我们还需要绑定scroll事件，scroll事件是伴随着大量计算的，会造成资源浪费，虽然我们可以通过节流函数来提高性能，但还是会有性能浪费的问题。而Intersection
          Observer可以不用监听scroll事件，做到元素一可见便调用回调，在回调里面我们来判断元素是否可见。
        </el-scrollbar>
      </div>
    </div>
    <div class="lazyload-lib">
      ...
      <div class="explain">
        <el-scrollbar>
          <a
            href="https://github.com/hilongjw/vue-lazyload#usage"
            target="_blank"
            >vue-lazyload文档地址</a
          >。阅读源码可以看出，该组件也是通过BoundingClientRect获取元素是否出现在视口，然后对出现在视口的图片再注册交叉观察器IntersectionObserve，来检测图片加载时机。相当于是对上述两种方式的综合应用。值得一提的是后面两种方式都需要指定图片的尺寸，否则在判断视口listener.checkInView()方法中会判定为所有图片均在视口中，也就不存在懒加载的概念了。
        </el-scrollbar>
      </div>
    </div>
  </div>
...
const inViewport = ($ele) => {
  return (
    ...
    $ele.getBoundingClientRect().top -
      jsRef.value.getBoundingClientRect().bottom <=
      offset
  );
};
...
const asyncLoadImg = async () => {
  for (let i = 0, len = jsUnLoadImageList.value.length; i < len; i++) {
    ...
    let flag = await inViewportWithResolve(
      $img,
      jsLoadedImageList.value[jsLoadedImageList.value.length - 1],
    );
    if (flag) {
...
onMounted(() => {
  ...
  observeUnLoadImageList.value = Array.from(
    observerRef.value.querySelectorAll("img"),
  );
