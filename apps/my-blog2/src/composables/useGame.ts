...
export function useGame(config: GameConfig) {
  ...
  function clickCube(cube: Cube) {
    ...
    // 判断插槽是否消除
    // 首先获取插槽中相同类型的方块
    const filterSlotsCubes = slots.value.filter(
      item => item.type === cube.type,
    )
    const length = filterSlotsCubes.length
    if (length === 2) {
      // 有二个相同方块，则消除所有方块
      const secondSlotsIndex = slots.value.findIndex(
      ...
      // 有一个相同方块，插入到相同方块后方
      const slotsIndex = slots.value.findIndex(
        item => item.type === cube.type,
      )
    ...
    layerLists.forEach((layerItems, index) => {
      ...
      layerItems.forEach(type => {
        ...
        previousCubes.forEach(cb => {
          if (
            Math.abs(cb.x - xy[0]) <= size
            && Math.abs(cb.y - xy[1]) <= size
          ) {
