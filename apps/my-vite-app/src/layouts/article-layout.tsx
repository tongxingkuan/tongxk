...
import { NBreadcrumb, NBreadcrumbItem, NDropdown, NLayout, NLayoutSider, NMenu, type MenuOption } from 'naive-ui'
    ...
    () => {
      ...
      const findKey = (menu: MenuOption[]): Key | undefined => {
        for (const item of menu) {
          if ((item.path as string[]).includes(router.currentRoute.value.path)) {
            return item.key
          }
          if (item.children) {
      ...
      currentTitle.value = findTitle(menu.value as (MenuOption & { labelText: string })[])
    },
    {
      immediate: true,
    }
