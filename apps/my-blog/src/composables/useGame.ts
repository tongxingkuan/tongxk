...
export function useGame(config: GameConfig) {
  const { container, callbacks, ...initialConfig } = {
    ...defaultConfig,
    ...config,
  }
  ...
  function clickCube(cube: Cube) {
    ...
    cube.status = 2
    const allIndex = allCubes.value.findIndex(item => item.id === cube.id)
    if (allIndex > -1)
      // 删除所有节点中改节点
      allCubes.value.splice(allIndex, 1)
    // 判断插槽是否消除
    // 首先获取插槽中相同类型的方块
    const filterSlotsCubes = slots.value.filter(item => item.type === cube.type)
    const length = filterSlotsCubes.length
    if (length === 2) {
      // 有二个相同方块，则消除所有方块
      const secondSlotsIndex = slots.value.findIndex(item => item.id === filterSlotsCubes[1].id)
      // 插入到相同方块后方
      ...
      // 有一个相同方块，插入到相同方块后方
      const slotsIndex = slots.value.findIndex(item => item.type === cube.type)
  ...
  function initGame(config: GameConfig) {
    ...
    const layerLists: number[][] = [] // 每一层所有的方块数量和方块类型
    let previousCubes: Cube[] = [] // 待检查的方块数组，用于检测方块遮挡
    const indexSet = new Set()
    const { layers, types, trap } = { ...initialConfig, ...config }
    const typesArr = new Array(types).fill(0).map((_, index) => index + 1)
    ...
    let layer = 1
    while (len <= itemLen) {
      const maxLayerCount = layer ** 2
      // 每层放置方块范围：平方的一半到平方
      const layerCount = ceil(random(maxLayerCount / 2, maxLayerCount))
      // splice返回删除的部分，原数组改变
