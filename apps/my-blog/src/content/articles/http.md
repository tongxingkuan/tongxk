...
`HTTP` 和 `HTTPS` 的最大区别是传输的数据是否加密。`HTTPS` 协议是由 `HTTP` 和 `SSL` 协议构建的可进行加密传输和身份认证的网络协议，比 `HTTP` 协议的安全性更高。也正是因为多了加密传输等过程， HTTP 协议在速度上比 HTTPS 快一些。 HTTP 端口号默认 `80` ， HTTPS 端口号默认 `443`。
...
4. 支持有限的并发请求
5. `Host` 请求头必填（支持虚拟主机，同一 IP 承载多个域名）
6. 新增 `PUT`、`DELETE`、`OPTIONS` 等方法
7. 支持 `Range` 请求头，可进行分块/断点续传（`206 Partial Content`）
