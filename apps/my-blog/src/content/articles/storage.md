...
会话存储。可由 **服务端**（通过 `Set-Cookie` 响应头）写入，也可由 **客户端**（通过 `document.cookie`，除非设置了 `HttpOnly`）写入；单条大小约 **4KB** 。客户端请求服务器时，即建立了会话，如果服务端需要记录客户端状态，则向客户端浏览器发送一个 cookie。
...
> - 在服务端，响应头中设置`Access-Control-Allow-Credentials`为 false，这样即使前端请求携带了 cookie 信息，服务器也不会读取 cookie。
...
遵循`同源 + 同窗口（tab）策略`，生命周期为当前标签页的会话期间：刷新页面、在同一 tab 内跳转仍然保留，关闭该标签页后失效。
