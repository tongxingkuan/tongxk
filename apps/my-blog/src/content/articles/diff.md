...
直接操作真实 DOM 的代价极高：一次节点操作可能触发浏览器的 `重排（Reflow）` 和 `重绘（Repaint）`，性能损耗远超 JS 运算。因此现代框架普遍采用 `虚拟 DOM`（JS 对象描述 DOM 结构），在内存中对比新老虚拟 DOM 树（即 Diff），计算出 `最小变更集（Patch）` 后再一次性应用到真实 DOM。

#### 为什么不用传统的树 Diff？

传统的两棵树完全 Diff 算法时间复杂度为 **O(n³)**（n 为节点数），其中包含：

- 遍历旧树 O(n)
- 对每个节点在新树中找匹配 O(n)
- 计算最小编辑距离（插入、删除、替换）O(n)

当 n = 1000 时需要 10 亿次计算，无法接受。React 和 Vue 都基于以下 **三大启发式假设**（tradeoff：放弃最优解，换取 O(n) 复杂度）将复杂度降到 **O(n)**：

1. **同层比较（Tree Diff）**：只对比同一层级的节点，跨层级移动会被视为「删除 + 创建」，不会递归匹配。实际业务中跨层级移动极少，这个 tradeoff 非常划算。
2. **相同类型才复用（Component/Element Diff）**：节点 `type` 不同直接销毁重建，不再深入比较子树。例如 `<div>` 变 `<p>` 时，即使子节点相同也会整棵子树重建。
3. **同层节点通过 `key` 标识**：没有 key 时按索引比对，有 key 时通过 key 建立映射实现 O(1) 查找复用，避免错误复用。

> 这三条假设共同将 Diff 从 O(n³) 降到 O(n)，是所有现代框架 Diff 算法的理论基石。
...
> **Fiber 深度解析**：Fiber 之前的 Stack Reconciler 是递归调用（深度优先遍历虚拟 DOM 树），一旦开始无法中断；当组件树很大时会长时间占用主线程（> 16.6ms），导致掉帧和卡顿。Fiber 将递归改为 `链表 + 循环`，每处理完一个 Fiber 节点就通过 `shouldYield()`（基于 `requestIdleCallback` / `MessageChannel` 实现的时间切片）判断是否让出主线程，从而实现 **可中断、可恢复、可优先级调度** 的更新。这正是 Concurrent Mode 的底层基础。
>
> Fiber 节点通过 `return`（指向父节点）、`child`（指向第一个子节点）、`sibling`（指向下一个兄弟节点）三个指针构成链表，但 **没有反向指针**，这是后续 Diff 无法使用双端比对的根本原因。
...
> **React Diff 核心要点**：
>
> - **单轮遍历 + 补充遍历**：第一轮正向遍历处理「顺序未变」的节点（实际业务的高频场景），这是最高效的路径；只有乱序时才进入第二轮，用 Map 实现 O(1) 查找。
> - **`lastPlacedIndex` 算法本质**：React 通过记录「最后一个可复用节点在旧链表中的索引」来判断移动。若当前节点的 `oldIndex < lastPlacedIndex`，说明它相对已处理节点「往左跑了」，需要右移（打 `Placement`）；否则不动，更新 `lastPlacedIndex = oldIndex`。这是一个 **贪心策略**，不保证最少移动次数。
> - **时间复杂度**：O(n)，但常数因子比 Vue3 大（没有最长递增子序列优化）。
> - **副作用收集**：Diff 阶段不直接操作 DOM，只在 Fiber 节点上打 `flags`（如 `Placement`、`Deletion`、`Update`），真正的 DOM 操作发生在后续的 `Commit` 阶段，保证 Render 阶段可中断。
...
| 维度             | React                  | Vue2                | Vue3                                  |
| ---------------- | ---------------------- | ------------------- | ------------------------------------- |
| 数据结构         | Fiber 单向链表         | 数组（VNode）       | 数组（VNode）                         |
| 遍历策略         | 单向遍历 + Map 查找    | 双端对比 + Map 查找 | 头尾预处理 + 最长递增子序列           |
| 移动判断         | `lastPlacedIndex` 贪心 | 双端交叉命中        | LIS 全局最优                          |
| 是否利用编译信息 | ❌（运行时）           | ❌（运行时）        | ✅（PatchFlag / Block Tree 静态标记） |
