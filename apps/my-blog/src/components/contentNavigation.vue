...
import { Document as DocumentIcon, Albums as QuestionMarkCircleIcon } from '@vicons/ionicons5'
import { NIcon, NLayout, NLayoutSider, NMenu, NSpace, type MenuOption } from 'naive-ui'
  ...
  () => {
    ...
    const findKey = (menu: MenuOption[]): Key | undefined => {
      for (const item of menu) {
        if ((item.path as string[])?.includes(useRouter().currentRoute.value.path)) {
          return item.key
        }
        if (item.children) {
    ...
    const findTitle = (menu: (MenuOption & { labelText: string })[]): string | undefined => {
      for (const item of menu) {
        if ((item.path as string[])?.includes(useRouter().currentRoute.value.path)) {
          return item.labelText
        }
        if (item.children) {
          const title = findTitle(item.children as (MenuOption & { labelText: string })[])
          if (title) {
            return title
          }
        }
    ...
    currentTitle.value = findTitle(menuOptions.value as (MenuOption & { labelText: string })[])
    nextTick(() => {
